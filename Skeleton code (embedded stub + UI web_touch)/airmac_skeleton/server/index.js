...
// Simple polling endpoint for pressure (simulated)
app.get('/api/cases/:id/status', (req, res) => {
  const c = cases.get(req.params.id);
  if (!c) return res.status(404).json({ error: 'Not found' });
  // Simulate pressure up/down around a target, with cap enforcement
  if (c.device.pumping) {
...
app.listen(PORT, () => console.log(`AIR MAC server running on :${PORT}`));
